...
def encrypt_file(key, src_path, dns_path, loop: asyncio.AbstractEventLoop, future: asyncio.Future):
...
def decrypt_file(key, src_path, dns_path, loop: asyncio.AbstractEventLoop, future: asyncio.Future):
    ...
    if os.path.getsize(src_path) <= NONCE_LENGTH + TAG_LENGTH + CHUNK_LENGTH_FOR_LARGE_FILE:
